    ...
    return (
      <div className={`flex items-center gap-2 ${className || ''}`}>
        <button
          ...
          className={`focus:ring-primary-500/20 relative inline-flex h-4 w-6 shrink-0 cursor-pointer rounded-full border-0 p-0.5 transition-colors duration-200 ease-in-out focus:ring-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 ${
            checked ? 'bg-primary-500' : 'bg-greyscale-200'
          }`}
          {...props}
        >
          {/* Sliding handle */}
