...
export function Sidebar() {
  ...
  const renderNavItem = (item: (typeof navigation)[0]) => {
    ...
    return (
      ...
      >
        <Icon name={item.icon} size="xl" className={cn(!collapsed && 'mr-3')} />
        {!collapsed && <span className="truncate">{item.name}</span>}
        {active && !collapsed && (
          <div className="bg-greyscale-900 absolute top-0 left-0 h-full w-1 rounded-r" />
        )}
      </NavLink>
    );
  };
  ...
  const renderGroupItem = (item: (typeof groups)[0]) => {
    return (
      ...
      >
        <Icon name={item.icon} size="lg" className={cn(!collapsed && 'mr-2')} />
        {!collapsed && <span className="truncate">{item.name}</span>}
      </NavLink>
    );
  };
      ...
      <div className="flex flex-grow flex-col overflow-y-auto">
        ...
        >
            ...
            {!collapsed && (
              <>
                <div className="min-w-0">
                  <p className="text-greyscale-900 truncate text-sm font-medium">
                    Workspace
                  </p>
                </div>
                <Icon
                  name="chevron-down"
                  size="lg"
                  className="text-greyscale-500 ml-1 shrink-0"
          ...
          <Button
            ...
            size="icon"
            className={cn(
              'border-sidebar-border shrink-0 rounded-full border',
          ...
          <div className="px-4 pb-3">
            <div className="relative">
              ...
              <Input
                type="search"
                placeholder="Search"
                className="border-greyscale-200 bg-greyscale-0 h-9 w-full rounded-lg border pl-10 text-sm"
              />
            </div>
          </div>
        )}
        ...
        >
          {/* Primary items */}
          <ul className={cn('space-y-1', !collapsed && 'px-2')}>
          ...
          <div className={cn(!collapsed && 'px-2')}>
            <button
              type="button"
              className={cn(
                'text-greyscale-500 flex w-full items-center gap-2 rounded-md px-2 py-1 text-xs font-medium tracking-wide uppercase',
                'hover:bg-sidebar-accent'
              )}
        ...
        {!collapsed && (
          <div className="px-4 pt-2 pb-3">
            <div className="bg-primary-500/5 rounded-2xl p-4 text-center">
              ...
              <p className="text-greyscale-900 mb-1 text-sm font-semibold">
                Get Asset save up to 25%
              </p>
              <p className="text-greyscale-500 mb-3 text-xs">
        ...
        >
          <div className={cn('flex flex-col gap-2', collapsed && 'gap-3')}>
            <button
              type="button"
              className={cn(
                'text-greyscale-500 flex items-center gap-2 rounded-md px-2 py-1 text-sm transition-colors',
                'hover:bg-sidebar-accent hover:text-sidebar-accent-foreground',
                collapsed && 'justify-center px-0'
              )}
            >
