...
function EngLeadUserManagement({
  ...
  const roles = ['client', 'developer', 'eng_lead'];
  const roleColors = {
    Client: 'bg-blue-100 text-blue-800',
    Developer: 'bg-green-100 text-green-800',
    'Network Lead': 'bg-purple-100 text-purple-800',
  ...
  const handleSaveUser = (e) => {
    ...
    if (editingUser) {
      onUserAction('edit', editingUser._id || editingUser.id, formData);
    } else {
  ...
  const handleDeleteUser = (user) => {
    onUserAction('delete', user._id || user.id);
